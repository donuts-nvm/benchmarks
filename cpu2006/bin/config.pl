...
#  All Rights Reserved
#
# $Id: config.pl 6697 2011-08-05 16:18:11Z CloyceS $
#
package Spec::Config;
...
my $version = '$LastChangedRevision: 6697 $ '; # Make emacs happier
...
EOCarp
	...
	# if so, then point the working reference pointer at that data.
	    ...
	    if ($name =~ /^(ext|mach|iterations|output_root|expid|flagsurl|temp_meter|power_analyzer)$/) {
		my $what = lc($1);
		...
		if ($reflist[0] != $me) {
		    if ($what =~ /^(?:ext|mach|output_root|expid|flagsurl|temp_meter|power_analyzer)$/) {
		...
		foreach my $ref (@reflist) {
		    ...
		    if ($op eq '+=') {
			if (defined($ref->{$name})) {
			    if (isa($ref->{$name},'HASH') && $ref->{$name}->{'op'} eq '+=') {
    ...
    if ($sysinfo_run && %sysinfo_dups) {
        ...
        ::Log(0, "  http://www.spec.org/cpu2006/Docs/config.html#sysinfo\n\n\n");
    }
        ...
        foreach my $thing ('bind', 'power_analyzer', 'temp_meter') {
            ...
            if (exists($fixup_needed{$thing})) {
                ...
                foreach my $section (keys %{$fixup_needed{$thing}}) {
                    ...
                    my $final = undef;
                    $final = $ref->{$thing} if isa($ref->{$thing}, 'ARRAY');
                    my %kill_keys = ();
...
sub get_sysinfo {
    ...
    if (exists($me->{'sysinfo_program'}) &&
        ...
        if (@infolines) {
            ...
            push @{$me->{'files_read'}}, $me->{'sysinfo_program'};
        }
        if (@infolines) {
...
sub bind {
    ...
    if ($me->tune eq 'base') {
        ...
        # basis for base runs
        my $config = $me->{'config'};
        $config = $::global_config unless ::isa($config, 'Spec::Config');
...
sub ranks {
    ...
    if ($tune eq 'base') {
        ...
        if (exists($config->{'clranks'}) && $config->{'clranks'} ne '') {
            ...
            my @sets = $config->benchmark_in_sets($me->benchmark);
            $rc = $config->default_lookup('ranks', $me->{'ext'}, $me->{'mach'}, @sets);
        }
        return $rc;
    } else {
        foreach my $check (qw(ranks clranks)) {
            my $tmp = $me->accessor_nowarn($check);
            next unless defined($tmp) && $tmp ne '';
            return $tmp;
        }
