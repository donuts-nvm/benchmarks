...
#  All Rights Reserved
#
# $Id: benchset.pm 6580 2011-06-30 18:27:36Z CloyceS $
#
...
my $version = '$LastChangedRevision: 6580 $ '; # Make emacs happier
...
sub report {
    ...
    if ($::lcsuite =~ /cpu(?:2006|v6)/) {
        ...
        $result->{'ompnumthreads'} = $config->{'ompnumthreads'};
    }
    ...
    if ($::lcsuite =~ /cpu(?:2006|v6)/) {
        ...
        push @{$result->{'do_dump'}}, 'ompnumthreads';
    }
    ...
    foreach my $sectionref (@::notes_info) {
        ...
        foreach my $tag (sort ::bytag @keys) {
            ...
            my ($key, $idx) = ($2, $3 + 0);
            $seen_tags{$note_tag}++;
            $result->{$note_tag} = {} unless exists($result->{$note_tag}) && isa($result->{$note_tag}, 'HASH');
    ...
    $result->{'notes'}->{''} = [] unless isa($result->{'notes'}->{''}, 'ARRAY');
    ...
    if (istrue($result->power)) {
        foreach my $item (qw(watts temp hum)) {
            foreach my $qty (qw(avg_ min_ max_)) {
                $result->{'idle_'.$qty.$item} = $config->{'idle_'.$qty.$item};
            }
        }
    }
	...
	::basepeak_munge($result, 0, @bp_bench);
      ...
      if ($result->rate) {
          $result->{'peakmean'} = $result->calc_mean_rate('peak');
      } else {
          $result->{'peakmean'} = $result->calc_mean_speed('peak');
      }
    }
    if (grep { /^base$/o } @{$config->tunelist}) {
      if ($result->rate) {
          $result->{'basemean'} = $result->calc_mean_rate('base');
      } else {
          $result->{'basemean'} = $result->calc_mean_speed('base');
      }
    }
