...
#  All Rights Reserved
#
# $Id: benchmark.pm 6540 2011-06-14 21:53:42Z CloyceS $
#
...
my $version = '$LastChangedRevision: 6540 $ '; # Make emacs happier
...
sub instance {
    ...
    $child->{'ranks'} = $child->ranks($tune) if ($::lcsuite eq 'mpi2007');
    ...
    if (!::isa($child->sources, 'ARRAY') && !::isa($child->sources, 'HASH')) {
...
sub workload_dirs {
    ...
    # files, add them here.
    if (::isa($me->{'workload_dirs'}->{$size}, 'ARRAY')) {
...
sub get_size_class {
    ...
    my @stuff = read_reftime($me->workload_dirs(jp($me->path, $me->datadir), $size, 'base'));
    return $stuff[2];
}

sub reference {
    my $me = shift;
    my ($ref, $size, $size_class) = read_reftime($me->workload_dirs(jp($me->path, $me->datadir), $me->size, 'base'));
    return 1 unless defined($ref);
    if ($size_class eq 'ref' && ($ref == 0)) {
	Log(0, "$size (ref) reference time for ".$me->descmode('no_size' => 1)." == 0\n");
	return 1;
    };
    return $ref;
}
...
sub read_reftime {
    ...
    Log(0, "read_reftime: 'reftime' could not be found in any of the following directories:\n   ".join("\n   ", @missing)."\n");
    return undef;
}
...
sub form_makefiles {
    ...
    if (istrue($me->feedback)) {
        ...
        @output_files = sort keys %{$filehash} if ::isa($filehash, 'HASH');
    }
	...
	push @{$vars{$exe}}, 'NEED_MATH='.$me->need_math;
        ...
        foreach my $chance ('ONESTEP', $benchlang[0].'ONESTEP') {
          ...
          if ($::lcsuite =~ /cpu(?:2006|v6)/ && $tune eq 'base' && istrue($onestep)) {
	...
	$vars{$exe} = join ("\n", @{$vars{$exe}}) . "\n";
        ...
        if (exists($me->deps->{$exe}) &&
            ::isa($me->deps->{$exe}, 'HASH')) {
            %objdeps = %{$me->deps->{$exe}};
        } else {
            %objdeps = %{$me->deps};
        }
        if (exists($me->srcdeps->{$exe}) &&
            ::isa($me->srcdeps->{$exe}, 'HASH')) {
...
sub compile_error_result {
    ...
    $result->{'ompnumthreads'} = 1 if $::lcsuite =~ /^omp20(01|12)$/;
    $result->{'rate'}      = 0;
    $result->{'submit'}    = 0;
    if (istrue($me->power)) {
        $result->{'avg_watts'} = 0;
...
sub do_parallel_setup {
    ...
    for(my $i = 0; $i < @dirs; $i++) {
        my $dir = $dirs[$i];
        next unless defined($dir) && ::isa($dir, 'HASH');
        my $pid = undef;
        if ($::running < $concurrent) {
	    ...
	    $command =~ s/\$SPECCOPYNUM/$i/g;
            Log(110, "\nAbout to exec \"$command\" for setup of ".$me->descmode."\n");

            ($start_time, $pid) = ::runspec_fork($me, \%::children, $i,
                                                 'loglevel' => 10,
                                                 'parent_msg' => "  Started child (\$pid) to setup copy $i for ".$me->descmode."\n",
                                                 'child_msg' => "  Setting up copy $i for ".$me->descmode."\n",
                                                 'log' => 1,
                                                );
            if ($pid) {
                chdir($top);
...
sub do_parallel_cleanup {
    ...
    for(my $i = 0; $i < @dirs; $i++) {
	    ...
	    $command =~ s/\$SPECCOPYNUM/$i/g;
            ...
            ($start_time, $pid) = ::runspec_fork($me, \%::children, $i,
                                                 'loglevel' => 10,
                                                 'parent_msg' => "  Started child (\$pid) for inter-run cleanup of copy $i for ".$me->descmode."\n",
                                                 'child_msg' => "  Doing inter-run cleanup of copy $i for ".$me->descmode."\n",
                                                 'log' => 1,
                                                );
            if ($pid) {
                chdir($top);
...
sub run_benchmark {
    ...
    # Munge the environment now so it can be stored in the command file
    my %oldENV = %ENV;
    main::munge_environment($me) if $env_vars;
    if ($::lcsuite !~ /^omp20(01|12)$/ && $::lcsuite !~ /^cpu2006/) {
        if (exists($ENV{'SPEC_CPU_NTHREADS'})) {
            $ENV{'OMP_NUM_THREADS'} = $ENV{'SPEC_CPU_NTHREADS'};
        } else {
	...
	Log(191, "Specinvoke: $command\n") unless istrue($me->fake);
        ...
        if (!$::from_runspec && istrue($me->power) && !$is_training) {
            my $isok = ::meter_start($me->benchmark, @{$me->powermeterlist});
            if (!$isok) {
                Log(0, "ERROR: Power meters could not be started\n");
                $result->{'valid'} = 'RE';
                push (@{$result->{'errors'}}, "power meters could not be started\n");
                return $result;
            }
            $isok = ::meter_start($me->benchmark, @{$me->tempmeterlist});
            if (!$isok) {
                Log(0, "ERROR: Temperature meters could not be started\n");
                $result->{'valid'} = 'RE';
                push (@{$result->{'errors'}}, "temperature meters could not be started\n");
                return $result;
            }
        }
	...
	%ENV = %oldENV if $env_vars;
        ...
        if (!$::from_runspec && istrue($me->power) && !$is_training) {
            ...
            if (!$isok) {
                Log(0, "ERROR: Power meters could not be stopped\n");
                $result->{'valid'} = 'RE';
                push (@{$result->{'errors'}}, "power meters could not be stopped\n");
                return $result;
            }
            ...
            if (!$isok) {
                ...
                $result->{'valid'} = 'RE';
                push (@{$result->{'errors'}}, "temperature meters could not be stopped\n");
                return $result;
            }
            ...
            my ($total, $avg, $min, $max, @list);

            # First, power:
            ($isok, $total, $avg, $min, $max, @list) = ::power_analyzer_watts(@{$me->powermeterlist});
            if (!$isok) {
                Log(0, "ERROR: Power meters could not be read\n");
                $result->{'valid'} = 'RE';
                push (@{$result->{'errors'}}, "power meters could not be read\n");
                return $result;
            }
            push @{$result->{'poweranalyzerrawrecords'}}, @list;
            $result->{'avg_watts'} = $total;
            $result->{'min_watts'} = $min;
            $result->{'max_watts'} = $max;

            # Now, temperature:
            ($isok, $avg, $min, $max, @list) = ::temp_meter_temp(@{$me->tempmeterlist});
            if (!$isok) {
                Log(0, "ERROR: Temperature meters could not be read (temperature)\n");
                $result->{'valid'} = 'RE';
                push (@{$result->{'errors'}}, "temperature meters could not be read (temperature)\n");
                return $result;
            }
            push @{$result->{'tempmeterrawrecords'}}, @list;
            $result->{'avg_temp'} = $avg;
            $result->{'min_temp'} = $min;
            $result->{'max_temp'} = $max;

            # Finally, humidity
            ($isok, $avg, $min, $max, @list) = ::temp_meter_humidity(@{$me->tempmeterlist});
            if (!$isok) {
                Log(0, "ERROR: Temperature meters could not be read (humidity)\n");
                $result->{'valid'} = 'RE';
                push (@{$result->{'errors'}}, "temperature meters could not be read (humidity)\n");
                return $result;
            }
            push @{$result->{'tempmeterrawrecords'}}, @list;
            $result->{'avg_hum'} = $avg;
		      ...
		      my $dirname = dirname($misname);
                  ...
                  if (@missing) {
                      ...
                      Log(0, '      '.join("\n      ", @missing)."\n".
                          ...
                          "    quickly that some output files were not even opened.  Check whether\n".
                          "    you have run out of memory or have run out of disk space.\n\n");
...
sub fdo_command_setup {
    ...
    $targets = [] unless ::isa($targets, 'ARRAY');
    my @targets = @$targets;
...
sub get_mandatory_option_md5_items {
    ...
    # CVT2DEV: $rc = "Non-makefile options:\nDEVELOPMENT TREE BUILD\n";
...
sub note_srcalts {
    ...
    foreach my $srcalt (@srcalts) {
        my $saref = $me->srcalts->{$srcalt};
        if (!defined($saref) || !::isa($saref, 'HASH')) {
            next unless $nocheck;
            $saref = { 'name' => $srcalt };
        }
