...
#  All Rights Reserved
#
# $Id: parse.pl 6459 2011-04-19 18:30:07Z MatColgrove $
...
my $version = '$LastChangedRevision: 6459 $ '; # Make emacs happier
...
sub resolve_user_selection {	## figure out which benchmarks are to be run
    ...
    if (@benchsets > 0) {
	    ...
	    # We'll just pick the size from the first benchset that has a
            ...
            my $class = $sizelist[0];
            $config->{'size'} = undef;
            for(my $i = 0; $i < @benchsets && !defined($config->{'size'}); $i++) {
                $config->{'size'} = $config->{'benchsets'}->{$benchsets[$i]}->{$class};
            }
