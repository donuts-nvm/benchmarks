//----------------------------  fe_tools.cc  ---------------------------
//    $Id: fe_tools.cc,v 1.3 2006/01/23 23:49:36 wolf Exp $
//    Version: $Name:  $
//
						   ...
						   FullMatrix<number> &difference_matrix)
{
  ...
  const unsigned int yan  = fe1.dofs_per_cell;
  FullMatrix<number> interpolation_matrix(yan);
...
{  
  ...
  AssertThrow (false, ExcInvalidFEName(name));
				   ...
				   // here after throwing
#ifdef SPEC_CPU
  return std::pair<FiniteElement<dim> *, unsigned int> (static_cast<FiniteElement<dim> *>(0),0);
#else
