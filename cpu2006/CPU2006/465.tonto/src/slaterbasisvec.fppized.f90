...
module SLATERBASISVEC_MODULE
   ...
   use SLATERBASIS_MODULE, only: read_keys_
   use SLATERBASIS_MODULE, only: clear_keys_
   use SLATERBASIS_MODULE, only: set_defaults_
   use SLATERBASIS_MODULE, only: nullify_ptr_part_
   use SLATERBASIS_MODULE, only: create_
   use SLATERBASIS_MODULE, only: keys_created_
   use SLATERBASIS_MODULE, only: put_
   use SLATERBASIS_MODULE, only: process_keys_
   use SLATERBASIS_MODULE, only: set_keys_
   use SLATERBASIS_MODULE, only: put_table_footer_
   use SLATERBASIS_MODULE, only: destroy_ptr_part_
   use SLATERBASIS_MODULE, only: put_table_header_
   use SLATERBASIS_MODULE, only: destroy_
   use SLATERBASIS_MODULE, only: copy_
   implicit none
...
!----------------------------------------------------------------------
   ...
   type(slaterbasis_type), dimension(:), pointer, private :: saved_self => NULL()
...
!  ******************
   ...
   subroutine create(self,dim)
    type(slaterbasis_type), dimension(:) :: self
   ...
   subroutine create_copy(self,vec)
    ...
    ! Create a replica copy of "vec".
      type(slaterbasis_type), dimension(:), intent(in) :: vec
      pointer :: self
    ! The following code is inherited from OBJECTVEC
   ...
   subroutine copy(self,vec)
    ...
    ! The following code is inherited from OBJECTVEC
      ...
      call ensure_(tonto,size(self)==size(vec),"SLATERBASISVEC:copy ... vec size does not match")
      do a = 1,size(vec)
...
! ***********************
   ...
   recursive subroutine read_list_keywords(self)
    ...
    ! The following code is inherited from OBJECTVEC
     ...
     call ensure_(tonto,next_item_(stdin)=="{","SLATERBASISVEC:read_list_keywords ... expecting open bracket symbol, {")
   ...
   subroutine read_data(self,ignore_braces)
    ...
    ! The following code is inherited from OBJECTVEC
     ...
     if (.not. present(ignore_braces)) then
        call ensure_(tonto,next_item_(stdin)=="{","SLATERBASISVEC:read_data ... expecting open bracket symbol, {")
     ...
     if (.not. present(ignore_braces)) then
        ...
        call ensure_(tonto,word=="}","SLATERBASISVEC:read_data ... expecting close bracket symbol, }")
     end if
   ...
   function data_length(self) result(length)
    ...
    ! must be terminated by a "}" symbol.
     ...
     integer(kind=kind(1)) :: length
    ! The following code is inherited from OBJECTVEC
     type(slaterbasis_type), pointer :: tmp
     character(128) :: word
     integer(kind=kind(1)) :: line,item

     call ensure_(tonto,next_item_(stdin)/="}","SLATERBASISVEC:data_length ... empty data list!")
     call read_(stdin,word)
     length = 0
   ...
   subroutine read_altered_data(self)
    ...
    ! The following code is inherited from OBJECTVEC
     ...
     call ensure_(tonto,associated(self),"SLATERBASISVEC:read_altered_data ... list does not exist yet")
     call ensure_(tonto,next_item_(stdin)=="{","SLATERBASISVEC:read_altered_data ... expecting open bracket symbol: {")
     call read_(stdin,word)
     read_loop: do
        call read_(stdin,word)
        if (word=="}") exit read_loop
        call ensure_(tonto,is_int_(word),"SLATERBASISVEC:read_altered_data ... expecting integer list-element index")
        s = to_int_(word)
        call ensure_(tonto,s<=size(self),"SLATERBASISVEC:read_altered_data ... list-element too large")
        call ensure_(tonto,s>0,"SLATERBASISVEC:read_altered_data ... list-element must be positive")
   ...
   call ensure_(tonto,associated(self),"SLATERBASISVEC:read_append_data ... list does not exist yet")
   call ensure_(tonto,next_item_(stdin)=="{","SLATERBASISVEC:read_append_data ... expecting open bracket symbol: {")
     nullify(saved_self)
   ...
   subroutine put_keys_table(self)
    ...
    ! The following code is inherited from OBJECTVEC
     ...
     call ensure_(tonto,keys_created_(self),"SLATERBASISVEC:put_keys_table ... no keys")
...
! ***************************
   ...
   recursive subroutine read_keywords(self)
    ...
    ! The following code is inherited from BASISVEC
     ...
     call ensure_(tonto,next_item_(stdin)=="{","SLATERBASISVEC:read_keywords ... expecting open bracket symbol, {")
     call read_(stdin,word)
   ...
   subroutine read_library_data(self,library,labels,n_unique_labels)
    ...
    ! The following code is inherited from BASISVEC
     ...
     character(128) :: word
     type(slaterbasis_type), pointer :: basis
     ...
     call ensure_(tonto,next_item_(stdin)=="{","SLATERBASISVEC:read_library_data ... expecting open bracket symbol, {")
     ...
     call ensure_(tonto,keys_created_(self),"SLATERBASISVEC:read_library_data ... no keys= supplied")
     call ensure_(tonto,word=="data=","SLATERBASISVEC:read_library_data ... no data= list keyword")
     call ensure_(tonto,next_item_(stdin)=="{","SLATERBASISVEC:read_library_data ... expecting open bracket symbol, {")
     call read_(stdin,word)             ! move past open brace
     ...
     do                           ! Try to match all basis labels
       ...
       call ensure_(tonto,next_item_(stdin)/="}","SLATERBASISVEC:read_library_data ... missing basis sets in library")
     end do
     call revert_(stdin)
     call destroy_(unique_labels)
...
!   ! labels in "labels" -- those which cannot be found already in "self".
!     self :: pointer
!     library :: STR, intent(in)
!     labels :: STRVEC, intent(in)
!     n_unique_labels :: integer(kind=kind(1)), intent(out), optional
!     unique_labels :: STRVEC*
!     word :: STR
!     basis :: ELEMENT_TYPE*
!     found,i,u,n_labels,N :: integer(kind=kind(1))
!     if (.not. .created) then
!        N = 0
...
!     call ensure_(tonto,stdin.next_item=="{","expecting open bracket symbol, {")
...
!     end
!     call ensure_(tonto,.keys_created,"no keys= supplied")
!     call ensure_(tonto,word=="data=","no data= list keyword")
!     call ensure_(tonto,stdin.next_item=="{","expecting open bracket symbol, {")
...
!       basis.destroy
!       if (found==n_labels) exit ! All bases found
!       call ensure_(tonto,stdin.next_item/="}","missing basis sets in library")
!     end
!     stdin.revert
!     unique_labels.destroy
!   end
...
!*******************************************************************************
   ...
   subroutine shrink(self,dim)
    ...
    ! The following code is inherited from OBJECTVEC
     type(slaterbasis_type), dimension(:), pointer :: old
     integer(kind=kind(1)) :: n

     call ensure_(tonto,associated(self),"SLATERBASISVEC:shrink ... no self array")
     call ensure_(tonto,dim<=size(self),"SLATERBASISVEC:shrink ... dim too large")
     call ensure_(tonto,dim>=0,"SLATERBASISVEC:shrink ... dim must be non-negative")
   ...
   subroutine expand(self,dim)
    ...
    ! The following code is inherited from OBJECTVEC
     ...
     if (.not. associated(self)) then
        ...
        call ensure_(tonto,dim>=size(self),"SLATERBASISVEC:expand ... dim not large enough")
        call ensure_(tonto,dim>=0,"SLATERBASISVEC:expand ... dim must be non-negative")
        old => self
        old_dim = size(old)
        nullify(self)
        call create_(self,dim)
   ...
   subroutine append(self,v)
    ...
    ! Expands self and appends the contents of vector "v".
     pointer :: self
     type(slaterbasis_type), dimension(:), intent(in) :: v
    ! The following code is inherited from OBJECTVEC
     integer(kind=kind(1)) :: dim
   ...
   subroutine append_1(self,value)
    ...
    ! Expands self by 1, and appends the single scalar "value" onto the end.
     pointer :: self
     type(slaterbasis_type), intent(in) :: value
    ! The following code is inherited from OBJECTVEC
     integer(kind=kind(1)) :: dim
