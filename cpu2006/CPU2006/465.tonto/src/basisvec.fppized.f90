...
module BASISVEC_MODULE
   ...
   use BASIS_MODULE, only: read_keys_
   use BASIS_MODULE, only: clear_keys_
   use BASIS_MODULE, only: set_defaults_
   use BASIS_MODULE, only: nullify_ptr_part_
   use BASIS_MODULE, only: create_
   use BASIS_MODULE, only: put_table_
   use BASIS_MODULE, only: keys_created_
   use BASIS_MODULE, only: set_keys_
   use BASIS_MODULE, only: destroy_ptr_part_
   use BASIS_MODULE, only: put_table_header_
   use BASIS_MODULE, only: destroy_
   use BASIS_MODULE, only: unnormalise_
   use BASIS_MODULE, only: put_
   use BASIS_MODULE, only: process_keys_
   use BASIS_MODULE, only: put_table_footer_
   use BASIS_MODULE, only: renormalise_
...
!----------------------------------------------------------------------
   ...
   public    maximum_basis_set_l_value_
   interface maximum_basis_set_l_value_
      module procedure maximum_basis_set_l_value
   end interface
   ...
   type(basis_type), dimension(:), pointer, private :: saved_self => NULL()
...
!  ******************
   ...
   subroutine create(self,dim)
    type(basis_type), dimension(:) :: self
   ...
   subroutine create_copy(self,vec)
    ...
    ! Create a replica copy of "vec".
      type(basis_type), dimension(:), intent(in) :: vec
      pointer :: self
    ! The following code is inherited from OBJECTVEC
   ...
   subroutine copy(self,vec)
    ...
    ! The following code is inherited from OBJECTVEC
      ...
      call ensure_(tonto,size(self)==size(vec),"BASISVEC:copy ... vec size does not match")
      do a = 1,size(vec)
...
! ***********************
   ...
   recursive subroutine read_list_keywords(self)
    ...
    ! The following code is inherited from OBJECTVEC
     ...
     call ensure_(tonto,next_item_(stdin)=="{","BASISVEC:read_list_keywords ... expecting open bracket symbol, {")
   ...
   subroutine read_data(self,ignore_braces)
    ...
    ! The following code is inherited from OBJECTVEC
     ...
     if (.not. present(ignore_braces)) then
        call ensure_(tonto,next_item_(stdin)=="{","BASISVEC:read_data ... expecting open bracket symbol, {")
     ...
     if (.not. present(ignore_braces)) then
        ...
        call ensure_(tonto,word=="}","BASISVEC:read_data ... expecting close bracket symbol, }")
     end if
   ...
   function data_length(self) result(length)
    ...
    ! must be terminated by a "}" symbol.
     ...
     integer(kind=kind(1)) :: length
    ! The following code is inherited from OBJECTVEC
     type(basis_type), pointer :: tmp
     character(128) :: word
     integer(kind=kind(1)) :: line,item

     call ensure_(tonto,next_item_(stdin)/="}","BASISVEC:data_length ... empty data list!")
     call read_(stdin,word)
     length = 0
   ...
   subroutine read_altered_data(self)
    ...
    ! The following code is inherited from OBJECTVEC
     ...
     call ensure_(tonto,associated(self),"BASISVEC:read_altered_data ... list does not exist yet")
     call ensure_(tonto,next_item_(stdin)=="{","BASISVEC:read_altered_data ... expecting open bracket symbol: {")
     call read_(stdin,word)
     read_loop: do
        call read_(stdin,word)
        if (word=="}") exit read_loop
        call ensure_(tonto,is_int_(word),"BASISVEC:read_altered_data ... expecting integer list-element index")
        s = to_int_(word)
        call ensure_(tonto,s<=size(self),"BASISVEC:read_altered_data ... list-element too large")
        call ensure_(tonto,s>0,"BASISVEC:read_altered_data ... list-element must be positive")
   ...
   call ensure_(tonto,associated(self),"BASISVEC:read_append_data ... list does not exist yet")
   call ensure_(tonto,next_item_(stdin)=="{","BASISVEC:read_append_data ... expecting open bracket symbol: {")
     nullify(saved_self)
   ...
   subroutine put_keys_table(self)
    ...
    ! The following code is inherited from OBJECTVEC
     ...
     call ensure_(tonto,keys_created_(self),"BASISVEC:put_keys_table ... no keys")
...
! ***************************
   ...
   recursive subroutine read_keywords(self)
    ...
    ! Read data from "stdin" using keyword style input.
     ...
     call ensure_(tonto,next_item_(stdin)=="{","BASISVEC:read_keywords ... expecting open bracket symbol, {")
     call read_(stdin,word)
...
!     word :: STR
!     basis :: ELEMENT_TYPE*
!     found,i,n_labels,N :: integer(kind=kind(1))
...
!   end
   ...
   subroutine read_library_data(self,library,labels,n_unique_labels)
    ...
    ! labels in "labels" -- those which cannot be found already in "self".
     ...
     call ensure_(tonto,next_item_(stdin)=="{","BASISVEC:read_library_data ... expecting open bracket symbol, {")
     ...
     call ensure_(tonto,keys_created_(self),"BASISVEC:read_library_data ... no keys= supplied")
     call ensure_(tonto,word=="data=","BASISVEC:read_library_data ... no data= list keyword")
     call ensure_(tonto,next_item_(stdin)=="{","BASISVEC:read_library_data ... expecting open bracket symbol, {")
     call read_(stdin,word)             ! move past open brace
     ...
     do                           ! Try to match all basis labels
       ...
       call destroy_(basis)
       if (found==n_labels) exit  ! All bases found
       call ensure_(tonto,next_item_(stdin)/="}","BASISVEC:read_library_data ... missing basis sets in library")
     end do
     call revert_(stdin)
     call destroy_(unique_labels)
...
!*******************************************************************************
   ...
   subroutine shrink(self,dim)
    ...
    ! The following code is inherited from OBJECTVEC
     type(basis_type), dimension(:), pointer :: old
     integer(kind=kind(1)) :: n

     call ensure_(tonto,associated(self),"BASISVEC:shrink ... no self array")
     call ensure_(tonto,dim<=size(self),"BASISVEC:shrink ... dim too large")
     call ensure_(tonto,dim>=0,"BASISVEC:shrink ... dim must be non-negative")
   ...
   subroutine expand(self,dim)
    ...
    ! The following code is inherited from OBJECTVEC
     ...
     if (.not. associated(self)) then
        ...
        call ensure_(tonto,dim>=size(self),"BASISVEC:expand ... dim not large enough")
        call ensure_(tonto,dim>=0,"BASISVEC:expand ... dim must be non-negative")
        old => self
        old_dim = size(old)
        nullify(self)
        call create_(self,dim)
   ...
   subroutine append(self,v)
    ...
    ! Expands self and appends the contents of vector "v".
     pointer :: self
     type(basis_type), dimension(:), intent(in) :: v
    ! The following code is inherited from OBJECTVEC
     integer(kind=kind(1)) :: dim
   ...
   subroutine append_1(self,value)
    ...
    ! Expands self by 1, and appends the single scalar "value" onto the end.
     pointer :: self
     type(basis_type), intent(in) :: value
    ! The following code is inherited from OBJECTVEC
     integer(kind=kind(1)) :: dim
