...
module IRREPVEC_MODULE
   ...
   use IRREP_MODULE, only: nullify_ptr_part_
   use IRREP_MODULE, only: destroy_ptr_part_
   use IRREP_MODULE, only: copy_
...
contains
   ...
   subroutine create(self,dim)
    type(irrep_type), dimension(:) :: self
    ! Create an irrepvec
      pointer :: self
...
!   end
   ...
   subroutine create_copy(self,vec)
    ...
    ! Create a replica copy of "vec".
      type(irrep_type), dimension(:), intent(in) :: vec
      pointer :: self
   ...
   subroutine copy(self,vec)
    ...
    ! The following code is inherited from OBJECTVEC
      ...
      call ensure_(tonto,size(self)==size(vec),"IRREPVEC:copy ... vec size does not match")
      do a = 1,size(vec)
