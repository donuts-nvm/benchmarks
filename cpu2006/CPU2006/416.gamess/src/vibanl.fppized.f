...
C
      ...
      DATA CLAB_STR/"   X","   Y","   Z"/
      CHARACTER*4 :: LETI_STR, IBLANK_STR
      DATA LETI_STR,IBLANK_STR/" I","  "/
      EQUIVALENCE (LETI, LETI_STR), (IBLANK, IBLANK_STR)
      CHARACTER*8 :: HSSIAN_STR
      EQUIVALENCE (HSSIAN, HSSIAN_STR)
      CHARACTER*8 :: VSCF_STR
      EQUIVALENCE (VSCF, VSCF_STR)
      DATA HSSIAN_STR/"HESSIAN "/, VSCF_STR/"VSCF    "/
C
...
C
  200 CONTINUE
      CALL CENMAS(NFRG,COM,ZMASST,CMASS)
C
      IF(GOTFRQ) GO TO 550
C
...
C jray: This function (STFASE) changes the phase of a vector
C by making the largest element +ive. This may cause VE with
C different optimization levels, since often times two elements
C are extremely close. So, the largest element may be different.
...
C
      WRITE(IGV,9070)
      IF(GOTADM) WRITE(IGV,9080)
      IF(.NOT.STATPT) WRITE(IGV,9010)
      IF(.NOT.SCFOK)  WRITE(IGV,9020)
      WRITE(IW,9070)
...
C
      IF(NFRG.GT.0) THEN
         ...
         DO 705 I=1,NFRG
            DO 700 J=1,NMPTS(I)
               IF(FMASS(J,I).GT.ZERO) NFGAT=NFGAT+1
  700       CONTINUE
  705    CONTINUE
         CALL VALFM(LOADFM)
         LFNM = 1    + LOADFM
...
C
  710 CONTINUE
      ...
      WRITE (IGV,9090)
      WRITE (IGV,9100) (J,J = MINCOL,MAXCOL)
      WRITE (IW,9090)
  ...
  720 CONTINUE
C
      WRITE(IGV,9110) (FREQ(J),LAB(J+1-MINCOL),J = MINCOL,MAXCOL)
      WRITE (IGV,9115)    (SCR(J,4),J=MINCOL,MAXCOL)
      IF(GOTDDM) WRITE (IGV,9120) (SCR(J,1),J=MINCOL,MAXCOL)
      IF(GOTADM) THEN
          WRITE(IGV,9130) (SCR(J,2),J=MINCOL,MAXCOL)
          WRITE(IGV,9140) (SCR(J,3),J=MINCOL,MAXCOL)
      END IF
      WRITE(IGV,9090)
...
C
      DO 730 IAT = 1,NAT
         I0 = 3*(IAT-1)
         WRITE(IGV,9150) IAT,ANAM(IAT),BNAM(IAT),
     *                  CLAB(1),(VEC(I0+1,J),J=MINCOL,MAXCOL)
         WRITE(IGV,9160) CLAB(2),(VEC(I0+2,J),J=MINCOL,MAXCOL)
         WRITE(IGV,9160) CLAB(3),(VEC(I0+3,J),J=MINCOL,MAXCOL)
  730 CONTINUE
C
...
C
      WRITE(IW,9090)
      WRITE(IGV,9090)
      WRITE(IGV,9180) CLAB(1),(SVTZT(1,I),I=MINCOL,MAXCOL)
      WRITE(IGV,9160) CLAB(2),(SVTZT(2,I),I=MINCOL,MAXCOL)
      WRITE(IGV,9160) CLAB(3),(SVTZT(3,I),I=MINCOL,MAXCOL)
      WRITE(IGV,9200) (SVTZTT(I),I=MINCOL,MAXCOL)
      ...
      WRITE(IGV,9090)
      WRITE(IGV,9190) CLAB(1),(SVTZR(1,I),I=MINCOL,MAXCOL)
      WRITE(IGV,9160) CLAB(2),(SVTZR(2,I),I=MINCOL,MAXCOL)
      WRITE(IGV,9160) CLAB(3),(SVTZR(3,I),I=MINCOL,MAXCOL)
      WRITE(IGV,9200) (SVTZRT(I),I=MINCOL,MAXCOL)
...
C
  900 CONTINUE
      CALL THERMO(IW,NIMAG,ZMASST,CC,COM,ZMS,NPART,NLAST,NSKIP)
...
C
 ...
 9180 FORMAT(" TRANS. SAYVETZ ",A4,9F12.8)
 9190 FORMAT("   ROT. SAYVETZ ",A4,9F12.8)
 9200 FORMAT(15X,"TOTAL",9F12.8)
...
C*MODULE VIBANL  *DECK CENMAS
      SUBROUTINE CENMAS(NFRG,COM,ZMASST,CMASS)
C
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
C
      DIMENSION CMASS(3),COM(3,*)
C
      PARAMETER (MXATM=500)
...
C
C
      CHARACTER*4 :: IBLANK_STR
      EQUIVALENCE (IBLANK, IBLANK_STR)
      CHARACTER*4 :: LETI_STR
      EQUIVALENCE (LETI, LETI_STR)
      DATA IBLANK_STR/"    "/,  LETI_STR/"   I"/
      CHARACTER*4 :: ICOOR_STR(7)
      EQUIVALENCE (ICOOR, ICOOR_STR)
      DATA ICOOR_STR/"STR.","BEND","TORS","OPBE","LIBE","5DIH","GTRS"/
C
...
C
 ...
 9010 FORMAT(//1X,79("-")/9X,'INTRINSIC VIBRATIONAL FREQUENCIES',
 ...
 9014 FORMAT( 1X,'INTERNAL COORDINATE',8X,'INTRINSIC FREQUENCIES'/
     *       33X,'(CM**-1)'/1X,50(1H-))
 9016 FORMAT(/1X,68("-")/1X,'INTERNAL COORDINATE',16X,'INTRINSIC FORCE',
     *           ' CONSTANTS'/
     *       28X,'(HARTREES/BOHR**2)',8X,'(MDYN/ANG)'/1X,68(1H-))
 9018 FORMAT(//6X,36("-")/6X,'TOTAL ENERGY DISTRIBUTION MATRIX M',
 ...
 9098 FORMAT(//10X,36("-")/10X,'NORMAL MODES IN INTERNAL',
...
C
      COMMON /IOFILE/ IR,IW,IPP,IS,IPK,IDAF,NAV,IODA(400)
      COMMON /PAR   / ME,MASTER,NPROC,IBTYP,IPTIM,GOPARR,DSKWRK,MASWRK
C
...
C*MODULE VIBANL  *DECK PRJGRD
      SUBROUTINE PRJGRD(GRAD,X,GTOT,RM,P,NATM3,NCCF)
C
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
C
...
C
      DIMENSION GRAD(NATM3),X(NCCF),GTOT(NCCF),RM(*),P(NCCF,NCCF)
...
C
C   THE ONLY NECESSARY INITIALIZED INPUTS ARE -GRAD- AND -NATM3- AND
C   -NCCF-, AND IN COMMON -DEFT- AND -TORQ-.  ON EXIT, THE AB INITIO
C   PART OF THE GRADIENT -GRAD- AND THE FRAGMENT COMPONENTS -DEFT-
C   AND -TORQ- WILL HAVE BEEN PROJECTED.
C   THE FOLLOWING ARE SCRATCH STORAGE:
C   X   : PROVIDE STORAGE FOR THE MASS-WEIGHTED COORDINATES, AND
...
C   P   : PROVIDE STORAGE FOR THE PROJECTOR MATRIX, INCLUDING
C         EFP COMPONENTS, NAMELY NCCF= 3*NAT + 6*NFRG.
C
C        THE STATUS OF THE EFP CODE, AS OF JULY 2, 2002 IS
C          A) SEE NOTES IN MIKE'S DFT FOLDER ABOUT THE NUMERICS
C          B) COMPLETE CONFIDENCE THAT $DATA PART IS CORRECT, BUT
...
C             PROJECTION AT ALL, SO THE NEXT IF STATEMENT JUST QUITS
...
C             TO USE M**(-1/2) AND M**(1/2) TO DO THIS (THE COMMENTS
C             IN -EFRMSS- ARE CLEARLY WRONG, AS THE CODE FILLS RM
C             WITH SETS OF THE POWERS OF THESE MOMENTS).  THE CONTENTS
C             OF THE -FRGMI- ARRAYS WERE NOT TESTED FOR CORRECTNESS.
C
      IF(NFRG.GT.0) RETURN
C
      NATM=NAT
...
C
      CALL RAMS(RM,1)
      IF(NFRG.GT.0) CALL EFRMSS(NFRG,RM(NATM3+1))
C
C       GET TOTAL MASS -TOTM- AND MASS-WEIGHTED COORDINATES -X-
C
      CALL CENMAS(NFRG,X,TOTM,SCR)
C
C       --- COMPUTE INERTIA TENSOR ---
C
      CALL VCLR(ROT,1,9)
...
C
      ...
      I=NATM3
      DO IFRG=1,NFRG
         GTOT(I+1)= RM(I+1) * DEFT(1,IFRG)
         GTOT(I+2)= RM(I+2) * DEFT(2,IFRG)
         GTOT(I+3)= RM(I+3) * DEFT(3,IFRG)
...
C
      I=NATM3
      DO IFRG=1,NFRG
         DEFT(1,IFRG)=X(I+1)/RM(I+1)
         DEFT(2,IFRG)=X(I+2)/RM(I+2)
         DEFT(3,IFRG)=X(I+3)/RM(I+3)
         TORQ(1,IFRG)=X(I+4)
         TORQ(2,IFRG)=X(I+5)
         TORQ(3,IFRG)=X(I+6)
         I=I+6
...
C
      DATA FIRST/.TRUE./
      CHARACTER*8 :: MASS_STR
      EQUIVALENCE (MASS, MASS_STR)
      CHARACTER*8 :: QNAM_STR
      EQUIVALENCE (QNAM, QNAM_STR)
      DATA MASS_STR/"MASS    "/, QNAM_STR/"AMASS   "/
C
...
C*MODULE VIBANL  *DECK THERMO
      SUBROUTINE THERMO(IW,NIMAG,ZMASST,CC,COM,ZMS,NPART,NLAST,NSKIP)
C
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      DOUBLE PRECISION KT,JPCAL
      LOGICAL LINEAR,PRTSCN,GOTFRQ,GOPARR,MASWRK,DSKWRK,ABEL,RTYPRT
      CHARACTER*8 FRGNME
      DIMENSION CC(3,NPART),COM(3,NPART),ZMS(NPART)
C
      PARAMETER (MXATM=500, MXFRG=50, MXPT=100, MXFGPT=MXPT*MXFRG)
C
      DIMENSION VMOI(3),ASVIB(3*MXATM)
C
      COMMON /EFMULT/ EFC(3,MXFGPT),EFCHG(2,MXFGPT),EFATRM(MXFGPT),
     *                EFBTRM(MXFGPT),EFATRM2(MXFGPT),EFBTRM2(MXFGPT),
     *                EFDIP(3,MXFGPT),EFQAD(6,MXFGPT),
...
C
      IF(PRTSCN) THEN
         WRITE(IW,9055)
         WRITE(IW,9060) (FREQ(I),ASVIB(I)*FACT,
  ...
  800 CONTINUE
      RETURN
C
 9000 FORMAT(/5X,31("-")/5X,'THERMOCHEMISTRY AT T=',F8.2,' K'/
     *    5X,31(1H-)//
 ...
 9050 FORMAT(1X,'THE HARMONIC ZERO POINT ENERGY IS (SCALED BY',
     ...
     *       1X,F15.6,' KCAL/MOL        ',
     *       1X,F15.6,' KJ/MOL')
 9055 FORMAT(11X,33("-")/11X,'VIBRATIONAL-ENTROPY DECOMPOSITION',/
     *    11X,33(1H-)//
