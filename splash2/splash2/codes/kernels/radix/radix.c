...

{
   ...
   key[0] = (int *) valloc(num_keys*sizeof(int));;
   key[1] = (int *) valloc(num_keys*sizeof(int));;
   key_partition = (int *) valloc((number_of_processors+1)*sizeof(int));;
   rank_partition = (int *) valloc((number_of_processors+1)*sizeof(int));;
...
/* POSSIBLE ENHANCEMENT:  Here is where one might distribute the key,
   ...
   for (i = 1; i < number_of_processors; i++) {
	...
	pthread_create(&ParmacsThreads[ParmacsThreadNum++], NULL, (void * (*)(void *))slave_sort, NULL);
}
   }
